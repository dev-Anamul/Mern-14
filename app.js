...
let progressBar, prgCount;

let progressIntv; //setInterval
start.addEventListener('click', function () {
  progressBar = time.value * 1;
  prgCount = time.value * 1;

  progressIntv = setInterval(() => {
    // progressBar = `${widthCal(progressBar)}`;
    progress.style.width = `${widthCal(prgCount, progressBar)}%`;
    ...
    if (progressBar == 0) {
      ...
      progressBar = time.value;
      progress.style.width = `0%`;
      time.value = '';
    } else {
      progressBar--;
      count.innerHTML =
        (progressBar < 10 ? '0' + progressBar : progressBar) + '%';
